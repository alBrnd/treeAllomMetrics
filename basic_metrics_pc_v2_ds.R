...
tree_height_pc <- function(pc,
  ...
  if (nrow(pc) == 0) {
    ...
    if (plot) {
      pc.table <- data.table::data.table(X=pc$X, Y=pc$Y, Z=pc$Z)
      pc_norm <- VoxR::filter_point_density(pc.table,0.05)
      ...
      plotXZ <- ggplot2::ggplot(pc_norm, ggplot2::aes(X, Z), col = plotcolors[1]) +
        ...
        ggplot2::theme(
          ...
          axis.ticks = ggplot2::element_line(size=0.8, color = "black"),
  ...
  if (plot) {
    if (is.nan(out_130$diameter)) {
      ...
      plotDBH <- ggplot2::ggplot() +
        ...
        ggplot2::coord_fixed(ratio = 1) +
        ggplot2::ggtitle(paste("DBH = NaN, ", "R2 = NaN, ", "fDBH = NaN", sep = "")) +
        ggplot2::scale_color_manual(
          name = "",
          values = c("points stem slice" = plotcolors[1]),
          guide = ggplot2::guide_legend(override.aes =
...
classify_crown_pc <-
  function(pc,
    ...
    if (!is.nan(dab)) {
      ...
      if (plot) {
        if (nrow(crown_pc) == 0) {
          pc.table <- data.table::data.table(X=trunk_pc$X, Y=trunk_pc$Y, Z=trunk_pc$Z)
          tree <- VoxR::filter_point_density(pc.table, 0.03)
          tree$class <- "trunk"
          ...
          X <- Y <- Z <- NULL
          if (nrow(trunk_pc) == 0) {
            pc.table <- data.table::data.table(X=crown$X, Y=crown$Y, Z=crown$Z)
            tree <- VoxR::filter_point_density(pc.table, 0.03)
            #tree <- crown
            tree$Z <- tree$Z - min(tree$Z)
            plotXZ <- ggplot2::ggplot(tree, ggplot2::aes(X, Z)) +
...
projected_area_pc <- function(pc,
  ...
  if (plot) {
    X <- Y <- NULL
    pc.table <- data.table::data.table(X=pc$X, Y=pc$Y, Z=pc$Z)
    pc_ds <- VoxR::filter_point_density(pc.table, 0.1)
