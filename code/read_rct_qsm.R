...
read_rct_qsm <- function(path, global = FALSE) {
  ...
  treedata <- list(
    ...
    "TreeHeight" = tree_data["height"], # Total tree height in meters
    "DBHqsm" = tree_data["DBH"], # Diameter at breast height
    "CrownRadius" = tree_data["crown_radius"], # Average crown radius
    "Dimension" = tree_data["dimension"], # Fractal dimension of branches
    "Monocotal" = tree_data["monocotal"], # Palm tree similarity metric
    "Bend" = tree_data["bend"], # Trunk bend metric
    "BranchSlope" = tree_data["branch_slope"] # Branch slope characteristics
  )
  # Prepare output
  out <- list(
    cylinder = cylinder,
