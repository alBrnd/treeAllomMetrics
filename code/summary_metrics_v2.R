  ...
  function(PC_path,
    ...
    if ("stem diameter" %in% metrics) {
      if (buttress) {
        ...
        tree$diameter_above_buttresses_m <- dab <- dab_out$dab
        tree$functional_diameter_above_buttresses_m <- fdab <- dab_out$fdab
        if (plot == TRUE) {
          d_plot <- dab_out$plot 
        ...
        tree$diameter_at_breast_height_m <- dbh <- dbh_out$dbh
        tree$functional_diameter_at_breast_height_m <- fdbh <- dbh_out$fdbh
        if (plot == TRUE) {
          d_plot <- dbh_out$plot
          d_plot <- d_plot + ggplot2::theme(legend.key=ggplot2::element_blank())
        }
    ...
    tree$D7 <- out_700$diameter
    if (taper) {
      taper_curve <- tryCatch({
        stem_tapering_pc(
          pc = pc,
          slice_thickness = slice_thickness,
          maxtaperheight = maxtaperheight,
          interval = interval,
          buttress = buttress,
          thresholdbuttress = thresholdbuttress,
          maxbuttressheight = maxbuttressheight,
        ...
        print('no taper curve :/')
        print(cond)
      }
      )
      #tree$taper <- taper_curve$T 
    }
    ...
    if ("alpha volume" %in% metrics) {
      av_out <- alpha_volume_pc(pc = pc, alpha = alpha, plot = FALSE)
      tree$alpha_volume_m3 <- av <- av_out$av
      text_av <- paste("AV =", round(av, 2), "m³")
    }
    ...
    if ("projected area" %in% metrics) {
      ...
      if (plot == TRUE) {
        text_pa <- paste("PA =", round(pa, 2), "m²")
        if ("alpha volume" %in% metrics) {
          pa_plot <- pa_out$plot +
            ggplot2::ggtitle(paste0(text_pa,'\n',text_av)) +
    ...
    if (is.character(OUT_path)) {
      if (plot) {
        ...
        file_plot_D7 <- paste0(OUT_path, "fig_D7_", tree_name, ".jpeg")
        file_plot_taper <- paste0(OUT_path, "fig_taper_", tree_name, ".jpeg")
        ...
        ggplot2::ggsave(filename = file_plot,
                        summary_fig, 
                        width = figure_width, 
        ...
        #saving the final summary figure
        ggplot2::ggsave(filename = file_plot_D7,
                        out_700$plot, 
        ...
        if (taper) {
          ggplot2::ggsave(filename = file_plot_taper,
                          taper_curve$plot, 
                          width = (figure_width/2), 
                          height = (figure_height/2), 
                          dpi = 600)
...
#'   fitted circle, the concave hull and the estimated center are colored by the
#'   first, second, third, fourth and fifth element of this list respectively.
#' @parallel Logical (default=FALSE), indicates if the function should be run in
#'   parallel on the trees.
#' @ncores Number of cores used in the parallel computation. Only relevant if
#'   parallel == TRUE.
#'
#' @return The summary of the basic structural metrics for multiple tree point
#'   clouds as a data.frame. Includes the tree height, diameter at breast
...
summary_basic_pointcloud_metrics <-
  function(PCs_path,
    ...
    if (parallel == TRUE) {
      ...
      parallel::clusterEvalQ(cl, {library(ITSMe)})
