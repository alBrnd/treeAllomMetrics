...
extract_rct_qsm_metrics <- function(inpath, outpath) {
  ...
  result_list <- lapply(qsmfiles, function(file) {
    ...
    data.frame(
      ...
      vol_cut007_rct = tree_volume_rct(treedata, cylinder, cylindercutoff = 0.07),
      vol_cut002_rct = tree_volume_rct(treedata, cylinder, cylindercutoff = 0.02),
      vol_trunk_rct = trunk_volume_rct(cylinder, cylindercutoff = 0.07),
      vol_trunk_cut007_rct = trunk_volume_rct(cylinder),
      vol_branch_cut007_rct = branch_volume_rct(cylinder, cylindercutoff = 0.07),
      vol_branch_cut002_rct = branch_volume_rct(cylinder, cylindercutoff = 0.02),
      vol_branch_all_rct = branch_volume_rct(cylinder, cylindercutoff = 0),
      sum_length_branch_cut007_rct = sum(cylinder$length[cylinder$BranchOrder > 0 & cylinder$diameter > 0.07]),
      mean_length_branch_cut007_rct = mean(cylinder$length[cylinder$BranchOrder > 0 & cylinder$diameter > 0.07]),
