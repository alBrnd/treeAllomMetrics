...
# calculate metrics with summary_basic_pointcloud_metrics_pertree
...
foreach(i = 1:n, .packages = c("ITSMe", "patchwork") ) %dopar%  {
  ...
  tryCatch({
    ...
    pc_metrics <- summary_basic_pointcloud_metrics_pertree(files_to_process[i], metrics = c(
      "stem diameter",
      "tree height",
      "projected area", "alpha volume"), # "alpha volume"
      crown = TRUE,
      plot = TRUE,
      OUT_path = outpath)
...
print("finished caluculating metrics")
...
# 
# joined_df <- left_join(basic_metrics, rct_metrics)
